...
% https://www.youtube.com/watch?v=K8nxTSPHZhs&list=PLR812eVbehlwEArT3Bv3UfcM9wR3AEZb5&index=8&ab_channel=KentComputing

% erl
% c('/home/alex/Desktop/expr.erl').

% PARSING: TEXT STRINGS => TREE REPRESENTATION
% expr:parse("(2+(3*4))").
% =>
...
% PRETTY PRINTING: TREE REPRESENTATION => TEXT STRINGS

% expr:print({add, {num, 2}, {mul, {num, 3}, {num, 4}}}).
% =>
% "(2+(3*4))"

% expr:print({add, {var, a}, {mul, {num, 2}, {var, b}}}).
% =>
% "(a+(2*b))"

% EVALUATION: COMPILATION + EXECUTION/RUNING

% COMPILING: TREE REPRESENTATION => BINARY SEQUENTIAL CODE
% expr:compile({add, {var, a}, {mul, {num, 2}, {var, b}}}).
% =>
% [{fetch,a},{push,2},{fetch,b},{mul2},{add2}]

% EXECUTION/RUNING: BINARY SEQUENTIAL CODE + ENVIRONMENT VARIABLES + STACK([]) => RESULT
% expr:run([{fetch,a},{push,2},{fetch,b},{mul2},{add2}], [{a, 1}, {b, 2}], []).
% =>
% 5

% SIMPLIFICATION: TREE REPRESENTATION + RULES OF REDUCING
% {Res, _} = expr:parse("(0+(1*x))"), expr:simplify(Res).
% =>
% {var,x}
